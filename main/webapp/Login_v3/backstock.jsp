								...
								<table class="table table-borderless table-shopping-cart">
									<thead class="text-muted">
										<tr class=" text-uppercase ">
											...
											<th scope="col" width="150">訂購數量</th>
											<th scope="col" width="150">成本</th>
											<th scope="col" width="150">訂購日期</th>
											<th scope="col" width="150">庫存數量</th>
											<!-- <th scope="col" class="text-right d-none d-md-block" width="200"></th>  多一列表 -->
										</tr>
									</thead>
									<tbody>
										<c:if test="${not empty iselect}">
											<c:forEach var="row" items="${iselect}">
												<c:url var="link" value="/Login_v3/backstock.jsp">
													<c:param name="id" value="${row.id }" />
													<c:param name="ingredients" value="${row.ingredients }" />
													<c:param name="unit" value="${row.unit }" />
													<c:param name="count" value="${row.count }" />
												</c:url>
												<tr class="border">
													<td>
														<div class="text-left ">
															<var class="price md-7">${row.ingredients }</var>
														</div>
													</td>
													<td>
														<div class="text-left ">
															<var class="price md-7">10</var>
														</div>
													</td>
													<td>
														<div class="text-left ">
															<var class="price md-7">$300</var>
														</div>
													</td>
													<td>
														<div class="text-left ">
															<var class="price md-7">2020/11/15</var>
														</div>
													</td>
													<td>
														<div class="text-left ">
															<var class="price md-7">${row.count }</var>
														</div>
													</td>
												</tr>
											</c:forEach>
